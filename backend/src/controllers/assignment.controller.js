...
export const createAssignment = asyncHandler(async (req, res) => {
    ...
    const studentIds = Array.isArray(rawStudentIds) ? rawStudentIds : rawStudentIds ? [rawStudentIds] : [];
    ...
    if (new Date(dueDate) < new Date()) throw new apiError(400, "Due date cannot be in the past");

    // Resolve target student IDs
    let targetStudentIds = [];

    if (studentIds && studentIds.length > 0) {
        // Assign to specific students — validate they are enrolled with this teacher
        const enrolledIds = (
        ...
        targetStudentIds = studentIds.filter(id => enrolledIds.includes(id.toString()));
